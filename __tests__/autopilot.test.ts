...
jest.mock('../src/helper/files', () => {
  ...
  return {
    ...
    dataFromXlsx: jest.fn(() => [
      ...
      ['TestShow', 'M:[1-12],D:[1-5],H:[14]', '', '', '', '', '', '', 'Talk'],
    ]),
...
describe('autopilot', () => {
  ...
  test('createFinishedHandler runs welocal -> nextcloud -> copyRepeat -> unlinkFile in order', async () => {
    ...
    expect(calls).toEqual(['welocal.upload', 'nextcloud.upload', 'copyRepeat', 'unlinkFile']);
  });
  ...
  test('welocal failure still attempts nextcloud and keeps the source file', async () => {
    const calls: string[] = [];
    ...
    const copyRepeatStub = jest.fn(() => calls.push('copyRepeat'));
    const unlinkFileStub = jest.fn(() => calls.push('unlinkFile'));
    ...
    // nextcloud still runs even though welocal threw...
    expect(uploaderNextcloud.upload).toHaveBeenCalledTimes(1);
    // ...but the source file is kept (not copied/unlinked) for a retry.
    expect(copyRepeatStub).not.toHaveBeenCalled();
    expect(unlinkFileStub).not.toHaveBeenCalled();
    expect(calls).toEqual(['welocal.upload', 'nextcloud.upload']);
  });
