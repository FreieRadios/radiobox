import { getFilename, getPath, writeFile, writeJsonFile } from '../helper/files';
...
export default class ScheduleExport {
  ...
  async toFTP(verbose?) {
    ...
    const targetFile = `${process.env.FTP_REMOTE_PATH}/${this.getFilename()}.json`;
    try {
      await client.access({
